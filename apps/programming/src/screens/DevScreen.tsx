...
function ValidateButton({ manifest }: { manifest: LabManifest }) {
  ...
  return (
    <div className="inline-flex items-center gap-2">
      ...
      {result && !result.ok && (
        <pre className="text-[10px] text-red-300 max-w-xs truncate">
          {result.error}
        </pre>
      )}
    </div>
...
export default function DevScreen() {
  const { uid } = useAuth();
  const { progress, resetProgress, getTotalCompleted } = useProgress(uid);

  const rendererCounts = countBy(labCatalog, (lab) => lab.rendererType);
  const difficultyCounts = countBy(labCatalog, (lab) => lab.difficulty);
  const trackCounts = countBy(labCatalog, (lab) => lab.track);
  const accessCounts = countBy(labCatalog, (lab) => lab.accessLevel);
  const totalScenarios = labCatalog.reduce(
    (sum, lab) => sum + lab.scenarios.length,
    0
  );

  const allRenderers: RendererType[] = [
    "action-rationale",
    "toggle-config",
    "investigate-decide",
    "triage-remediate",
  ...
  return (
    <div className="min-h-screen bg-[#1E293B] text-gray-300 p-4 pb-24">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-[#1E293B] border border-gray-600 rounded-lg p-4 mb-6 space-y-4">
          <h2 className="text-sm font-semibold text-white">Content Coverage</h2>

          <div>
            <h3 className="text-[10px] font-bold text-gray-500 uppercase mb-1">
              By Renderer ({totalScenarios} total scenarios)
            </h3>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(difficultyCounts).map(([key, value]) => (
                <span
                  key={key}
                  className="px-2 py-1 rounded text-xs font-mono bg-gray-700 text-gray-300"
                >
                  {key}: {value}
                </span>
              ))}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(trackCounts).map(([key, value]) => (
                <span
                  key={key}
                  className="px-2 py-1 rounded text-xs font-mono bg-gray-700 text-gray-300"
                >
        ...
        <div className="bg-[#1E293B] border border-gray-600 rounded-lg p-4 mb-6 space-y-3">
          ...
          <pre className="text-[10px] text-gray-400 bg-[#0f172a] rounded p-3 overflow-auto max-h-48 font-mono">
        ...
        <div className="space-y-3">
          {labCatalog.map((lab) => (
            <div
              key={lab.id}
              className="bg-[#1E293B] border border-gray-600 rounded-lg p-4 space-y-2"
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-1 text-[10px] font-mono">
                    ...
                    <span className="px-1.5 py-0.5 rounded bg-gray-700 text-[#2A7F6F]">
                      {lab.rendererType}
                    </span>
                    <span
