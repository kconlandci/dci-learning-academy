// ============================================================
// DCI IT Support Labs — Usage Stats Screen
// ============================================================
...
export default function AnalyticsScreen() {
  ...
  const handleClear = async () => {
    ...
    try {
      ...
      confirmed = window.confirm("Clear Analytics?\n\nThis will delete all usage tracking data. This cannot be undone.");
    }
    if (confirmed) { clearAnalytics(); setSummary(getAnalyticsSummary()); }
  };
    ...
    <div className="min-h-screen bg-white p-4 pb-24">
      <div className="max-w-lg mx-auto pt-6">
        <div className="flex items-center gap-3 mb-6">
          <button onClick={() => window.history.length > 1 ? navigate(-1) : navigate("/")} aria-label="Go back" className="w-10 h-10 rounded-xl bg-[#F5F5F5] flex items-center justify-center active:bg-gray-200 transition-colors">
            <ArrowLeft size={18} className="text-gray-500" />
          </button>
          <div><h1 className="text-lg font-bold text-[#1A1A1A]">Usage Stats</h1><p className="text-xs text-gray-400">Local analytics data</p></div>
        </div>
        <div className="grid grid-cols-2 gap-3 mb-6">
          <StatCard icon={<BarChart3 size={16} className="text-blue-400" />} label="Sessions" value={String(summary.totalSessions)} />
          <StatCard icon={<Clock size={16} className="text-purple-400" />} label="Avg Session" value={formatDuration(summary.avgSessionSeconds)} />
          <StatCard icon={<Trophy size={16} className="text-sky-400" />} label="Avg Score" value={summary.labsCompleted > 0 ? `${summary.avgScore}/100` : "—"} />
            ...
            <div className="space-y-2">
              {summary.topLabs.map((lab, i) => (
                <div key={lab.labId} className="flex items-center gap-3">
                  <span className="w-5 h-5 rounded-full bg-gray-200 flex items-center justify-center text-[10px] text-gray-500 font-bold shrink-0">{i + 1}</span>
                  <span className="text-xs text-[#1A1A1A] flex-1 truncate">{getLabTitle(lab.labId)}</span>
        ...
        <button onClick={handleClear} className="flex items-center gap-3 w-full bg-[#F5F5F5] rounded-xl p-4 min-h-[48px] active:bg-gray-200 transition-colors text-left">
          <Trash2 size={16} className="text-red-400" /><span className="text-sm font-medium text-red-400">Clear Analytics Data</span>
        </button>
      </div>
    </div>
  );
