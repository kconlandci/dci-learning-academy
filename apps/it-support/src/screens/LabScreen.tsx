...
export default function LabScreen() {
  ...
  const completedLabIds = Object.keys(progress.labs).filter((id) => progress.labs[id]?.completed);
  const recommendation = useRecommendedLab(labId ?? "", completedLabIds);
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-xs text-gray-500 mb-4">This scenario requires Founders Pack access.</p>
  ...
  const handleLabComplete = (score: number, history: GradeResult[]) => {
    recordLabCompletion(manifest.id, score, history.map((h, i) => ({ scenarioIndex: i, grade: h.type })));
    ...
    const totalCompleted = Object.values(progress.labs).filter((l) => l.completed).length + (alreadyCounted ? 0 : 1);
  ...
  const renderUpNext = () => {
    if (!recommendation) {
      return (
        <div className="bg-[#F5F5F5] rounded-xl p-4 text-center">
          ...
          <p className="text-sm font-semibold text-[#1A1A1A] mb-1">You've completed all available scenarios!</p>
          <p className="text-xs text-gray-500 mb-3">More scenarios coming soon. Keep your streak alive by replaying favorites.</p>
          <button onClick={() => navigate("/")} className="px-4 py-2 rounded-lg bg-amber-500 text-white text-sm font-medium min-h-[44px]">Browse All Scenarios</button>
        </div>
      );
    }
    ...
    return (
      <div className="bg-[#F5F5F5] rounded-xl p-4">
        <p className="text-[10px] text-gray-400 uppercase tracking-wide font-semibold mb-1">Up Next</p>
        ...
        {isRecPremium ? (
          ...
          <button onClick={() => navigate(`/lab/${recommendation.lab.id}`)} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-amber-500 text-white text-sm font-medium min-h-[44px]">Start Scenario <ArrowRight size={14} /></button>
        )}
      </div>
    );
  };
  ...
  return (
    <ErrorBoundary>
      <LabEngine key={manifest.id} manifest={manifest} renderer={Renderer} onExit={() => navigate("/")} onLabComplete={handleLabComplete} onHintUsed={handleHintUsed} renderAfterResults={renderUpNext} />
    </ErrorBoundary>
  );
}
