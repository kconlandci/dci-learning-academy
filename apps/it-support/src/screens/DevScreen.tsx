// ============================================================
// DCI IT Support Labs Dev Preview Screen — /dev
// ============================================================
...
function ValidateButton({ manifest }: { manifest: LabManifest }) {
  ...
  return (
    <div className="inline-flex items-center gap-2">
      <button onClick={validate} className="px-3 py-1.5 rounded bg-gray-200 text-[#1A1A1A] text-xs font-medium hover:bg-gray-300 transition-colors">Validate</button>
      {result !== null && <span className={`text-xs font-mono ${result.ok ? "text-green-400" : "text-red-400"}`}>{result.ok ? "PASS" : "FAIL"}</span>}
      {result && !result.ok && <pre className="text-[10px] text-red-300 max-w-xs truncate">{result.error}</pre>}
    </div>
  );
}
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 mb-6 space-y-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {allRenderers.map((r) => (
                <span key={r} className={`px-2 py-1 rounded text-xs font-mono ${(rendererCounts[r] || 0) < 2 ? "bg-yellow-900/30 text-yellow-600 border border-yellow-500/30" : "bg-[#F5F5F5] text-gray-500"}`}>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">{Object.entries(difficultyCounts).map(([k, v]) => (<span key={k} className="px-2 py-1 rounded text-xs font-mono bg-[#F5F5F5] text-gray-500">{k}: {v}</span>))}</div>
          </div>
          <div>
            <h3 className="text-[10px] font-bold text-gray-400 uppercase mb-1">By Track</h3>
            <div className="flex flex-wrap gap-2">{Object.entries(trackCounts).map(([k, v]) => (<span key={k} className="px-2 py-1 rounded text-xs font-mono bg-[#F5F5F5] text-gray-500">{k}: {v}</span>))}</div>
          </div>
          <div>
            <h3 className="text-[10px] font-bold text-gray-400 uppercase mb-1">By Access Level</h3>
            <div className="flex flex-wrap gap-2">{Object.entries(accessCounts).map(([k, v]) => (<span key={k} className="px-2 py-1 rounded text-xs font-mono bg-[#F5F5F5] text-gray-500">{k}: {v}</span>))}</div>
          </div>
        </div>
        <div className="bg-white border border-gray-200 rounded-lg p-4 mb-6 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={resetProgress} className="px-3 py-1.5 rounded bg-red-900/30 text-red-400 text-xs font-medium hover:bg-red-900/50 transition-colors border border-red-500/30">Reset Progress</button>
          </div>
          ...
          <pre className="text-[10px] text-gray-500 bg-slate-950 rounded p-3 overflow-auto max-h-48 font-mono">{JSON.stringify(progress, null, 2)}</pre>
        </div>
