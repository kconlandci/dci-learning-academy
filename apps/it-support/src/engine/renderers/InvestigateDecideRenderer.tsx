    ...
    <div className="space-y-4">
      ...
      <div>
        <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-2 flex items-center gap-2">
          <Search size={14} /> Investigation Sources
        </h3>
          ...
          {s.investigationData.map((src) => {
            ...
            return (
              <div key={src.id} className="rounded-xl border-2 border-gray-200 bg-[#F5F5F5] overflow-hidden">
                <button onClick={() => toggleSource(src.id)} disabled={isLocked}
                  className={`w-full min-h-[44px] px-4 py-3 flex items-center justify-between text-left transition-all ${isLocked ? "cursor-default" : "cursor-pointer active:bg-gray-200/50"}`}
                >
                  <div className="flex items-center gap-2">
                    {isViewed ? <Eye size={14} className="text-sky-400" /> : <EyeOff size={14} className="text-gray-400" />}
                    <span className={`text-sm font-medium ${isViewed ? "text-[#1A1A1A]" : "text-gray-500"}`}>{src.label}</span>
                  </div>
                  {src.isCritical && isLocked && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-500/20 text-red-300 font-medium">KEY</span>
                  )}
                </button>
                {isExpanded && (
                  <div className="px-4 pb-3 border-t border-gray-200">
                    <pre className="text-xs text-green-400 font-mono whitespace-pre-wrap leading-relaxed mt-2 bg-white/50 rounded-lg p-3">{src.content}</pre>
                  </div>
                )}
              </div>
            );
      ...
      <div>
        <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-2">Choose Action</h3>
        <div className="grid gap-2">
          {s.actions.map((action) => {
            const isSelected = selectedAction === action.id;
            return (
              <button key={action.id} onClick={() => !isLocked && setSelectedAction(action.id)} disabled={isLocked}
                className={`min-h-[44px] p-3 rounded-xl border-2 text-left text-sm font-medium transition-all ${
                  isSelected ? "bg-sky-500/20 border-sky-500 text-sky-200" : "bg-[#F5F5F5] border-gray-200 text-gray-500 hover:border-gray-400"
      ...
      <div>
        <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-2">Justify Your Decision</h3>
        <div className="grid gap-2">
          {s.rationales.map((r) => {
            const isSelected = selectedRationale === r.id;
            return (
              <button key={r.id} onClick={() => !isLocked && setSelectedRationale(r.id)} disabled={isLocked}
      ...
      {phase === "active" && (
        <button onClick={handleSubmit} disabled={!selectedAction || !selectedRationale}
          className="w-full py-3 rounded-xl bg-sky-500 hover:bg-sky-600 disabled:bg-gray-200 disabled:text-gray-400 text-[#1A1A1A] font-semibold transition-colors"
        ...
        <div className="space-y-3">
          ...
          {missedCritical.length > 0 && (
            <div className="rounded-xl p-3 bg-red-500/10 border border-red-500/30">
              <p className="text-xs text-red-300">You missed key evidence: {missedCritical.map((s) => s.label).join(", ")}</p>
            </div>
          )}
        </div>
      )}
