...
const COLOR_MAP: Record<string, { selected: string; base: string }> = {
  ...
  yellow: { selected: "bg-yellow-500/20 border-yellow-500 text-yellow-600", base: "bg-[#F5F5F5] border-gray-200 text-gray-500 hover:border-gray-400" },
  orange: { selected: "bg-orange-500/20 border-orange-500 text-orange-300", base: "bg-[#F5F5F5] border-gray-200 text-gray-500 hover:border-gray-400" },
...
export default function ActionRationaleRenderer({ scenario, onSubmit, phase, feedbackResult }: RendererProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {s.displayFields && s.displayFields.length > 0 && (
      ...
      {s.evidence && s.evidence.length > 0 && (
        <div className="bg-[#1E293B] rounded-xl p-4 font-mono text-xs text-green-400 space-y-1">
          {s.evidence.map((e, i) => (<div key={i}>&rarr; {e}</div>))}
        </div>
      )}
      {s.logEntry && (
        <div className="bg-[#1E293B] rounded-xl p-3 font-mono text-xs text-gray-500 whitespace-pre-wrap">{s.logEntry}</div>
      )}
      <div>
          ...
          {s.actions.map((action) => {
            ...
            return (
              <button key={action.id} onClick={() => !isLocked && setSelectedAction(action.id)} disabled={isLocked}
                className={`p-3 rounded-xl border-2 text-left text-sm font-medium transition-all ${isSelected ? colors.selected : colors.base} ${isLocked ? "opacity-70 cursor-default" : "cursor-pointer"}`}
              >{action.label}</button>
            );
          })}
        </div>
