    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="space-y-2">
          {s.evidence.map((ev, i) => (
            <div key={i} className="bg-[#1E293B] rounded-xl p-3 border border-gray-200">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{ev.type}</span>
              <pre className="text-xs text-green-400 font-mono whitespace-pre-wrap leading-relaxed mt-1">{ev.content}</pre>
            </div>
          ))}
        </div>
      </div>
      <div>
        <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-2">Step 1 — Classify the Issue</h3>
        <div className="grid gap-2">
          {s.classifications.map((c) => {
            const isSelected = selectedClassification === c.id;
            return (
              <button key={c.id} onClick={() => !isLocked && setSelectedClassification(c.id)} disabled={isLocked}
                className={`min-h-[44px] p-3 rounded-xl border-2 text-left transition-all ${
                  isSelected ? "bg-blue-500/20 border-blue-500 text-blue-200" : "bg-[#F5F5F5] border-gray-200 text-gray-500 hover:border-gray-400"
      ...
      <div>
        <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-2">Step 2 — Select Remediation</h3>
        <div className="grid gap-2">
          {s.remediations.map((r) => {
            const isSelected = selectedRemediation === r.id;
            return (
              <button key={r.id} onClick={() => !isLocked && setSelectedRemediation(r.id)} disabled={isLocked}
                className={`min-h-[44px] p-3 rounded-xl border-2 text-left transition-all ${
      ...
      <div>
        <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-2">Step 3 — Justify Your Decision</h3>
        <div className="grid gap-2">
          {s.rationales.map((r) => {
      ...
      {phase === "active" && (
        <button onClick={handleSubmit} disabled={!selectedClassification || !selectedRemediation || !selectedRationale}
