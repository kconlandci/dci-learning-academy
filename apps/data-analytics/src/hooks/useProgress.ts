...
const DciDataAnalyticsProgressSchema = z.object({
...
type DciDataAnalyticsProgress = z.infer<typeof DciDataAnalyticsProgressSchema>;

const STORAGE_KEY = "dci-data-analytics_progress";
const PREFS_KEY = "dci-data-analytics_progress";
...
function createDefaultProgress(): DciDataAnalyticsProgress {
...
function validateProgressShape(data: unknown): DciDataAnalyticsProgress | null {
  const result = DciDataAnalyticsProgressSchema.safeParse(data);
  if (result.success) return result.data;
  console.warn("[DCI Data Analytics Labs] Progress validation failed:", result.error.message);
  return null;
}

function migrateProgress(data: DciDataAnalyticsProgress): DciDataAnalyticsProgress {
...
function loadProgress(): DciDataAnalyticsProgress {
  try {
    ...
    if (raw) {
      ...
      if (!validated) {
        console.warn("[DCI Data Analytics Labs] Progress data failed validation — resetting.");
...
function saveProgress(progress: DciDataAnalyticsProgress) {
  progress.updatedAt = new Date().toISOString();
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(progress));
  } catch (error) {
    console.error("[DCI Data Analytics Labs] Failed to save progress:", error);
    try {
      const minimal = {
        ...progress,
      ...
      console.error("[DCI Data Analytics Labs] Even minimal save failed. Progress may be lost.");
    }
  }
}

function mirrorToPreferences(progress: DciDataAnalyticsProgress) {
  Preferences.set({
    key: PREFS_KEY,
    value: JSON.stringify(progress),
  }).catch((err) => {
    console.warn("[DCI Data Analytics Labs] Failed to mirror progress to Preferences:", err);
  });
}

async function recoverFromPreferences(): Promise<DciDataAnalyticsProgress | null> {
  try {
    ...
    if (value) {
      ...
      if (validated) {
        const migrated = migrateProgress(validated);
        console.info("[DCI Data Analytics Labs] Recovered progress from Preferences backup.");
        return migrated;
      }
    }
  } catch {
...
export function useProgress(userId?: string | null) {
  const [progress, setProgress] = useState<DciDataAnalyticsProgress>(loadProgress);
      ...
      setProgress((prev) => {
        ...
        const dciStudentId =
          typeof localStorage !== "undefined"
            ...
            : null;
        if (dciStudentId) {
          recordToFirestore(dciStudentId, "data-analytics", labId).catch(
            (err) => {
              console.warn(
...
export type { DciDataAnalyticsProgress, LabProgress, ScenarioResult };
