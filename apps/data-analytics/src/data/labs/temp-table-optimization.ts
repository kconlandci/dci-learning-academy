...
export const tempTableOptimizationLab: LabManifest = LabManifestSchema.parse({
  "schemaVersion": "1.1",
  "id": "temp-table-optimization",
  "version": 1,
  "title": "Temp Table Optimization",
  "tier": "intermediate",
  ...
  "description": "Practice temp table optimization inside a realistic analytics workflow, choose the safest decision, and explain why it is the most trustworthy SQL or data move.",
  "estimatedMinutes": 14,
  "learningObjectives": [
    "Clarify the stakeholder need behind temp table optimization.",
  ...
  "scenarios": [
    {
      "type": "toggle-config",
      "id": "temp-table-optimization-scenario-1",
      "title": "Configure the default quality controls",
      "description": "You are preparing a shared asset for temp table optimization. Set the defaults so the warehouse engineer gets trustworthy results without relying on tribal knowledge.",
      "targetSystem": "warehouse.compute_usage.semantic_model",
      "items": [
        {
          "id": "grain-check",
      ...
      "feedback": {
        "perfect": "Great configuration. Temp Table Optimization becomes safer to reuse when quality checks, null policies, and reviews are explicit.",
    ...
    {
      "type": "toggle-config",
      "id": "temp-table-optimization-scenario-2",
      "title": "Set access and change controls",
      "description": "A wider audience wants self-service access to temp table optimization. Configure the model so speed does not outrun governance.",
      "targetSystem": "warehouse.compute_usage.reporting_view",
      "items": [
        {
          "id": "access-scope",
          "label": "Access Scope",
    ...
    {
      "type": "toggle-config",
      "id": "temp-table-optimization-scenario-3",
      "title": "Tune the publish workflow",
      "description": "The team wants a faster release cycle for temp table optimization. Configure the publish workflow so the team can move quickly without normalizing broken output.",
      "targetSystem": "warehouse.compute_usage.publish_job",
      "items": [
        {
          "id": "test-suite",
  ...
  "hints": [
    "Start by naming the business grain that temp table optimization should produce.",
  ...
  "careerInsight": "Advanced analytics work often separates solid SQL users from the engineers who can keep an entire warehouse fast and stable. Temp Table Optimization is a common scenario in interviews, production reviews, and cross-functional planning.",
  "toolRelevance": [
    "EXPLAIN plans",
    "Snowflake",
    "PostgreSQL"
