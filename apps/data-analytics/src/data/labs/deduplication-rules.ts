...
export const deduplicationRulesLab: LabManifest = LabManifestSchema.parse({
  "schemaVersion": "1.1",
  ...
  "description": "Practice deduplication rules inside a realistic analytics workflow, choose the safest decision, and explain why it is the most trustworthy SQL or data move.",
  "estimatedMinutes": 10,
  "learningObjectives": [
    "Clarify the stakeholder need behind deduplication rules.",
  ...
  "scenarios": [
    {
      ...
      "id": "deduplication-rules-scenario-1",
      "title": "Choose the strongest SQL approach",
      "context": "data ops lead needs a reliable answer for deduplication rules before the weekly review. Pick the action that keeps the query auditable and the result grain stable under deadline pressure.",
      "displayFields": [
        {
          "label": "Dataset",
      ...
      "feedback": {
        "perfect": "Excellent. Deduplication Rules is strongest when you keep the grain explicit and validate the joins before sharing a metric.",
        ...
        "wrong": "That choice adds avoidable risk. For deduplication rules, the best move is the option that stays reviewable and grain-aware."
      }
    },
    {
      "type": "action-rationale",
      "id": "deduplication-rules-scenario-2",
      "title": "Handle the stakeholder follow-up",
      "context": "After your first draft, the data ops lead asks for one more slice of deduplication rules by region. Decide how to respond without turning a clean query into a fragile one-off.",
      "displayFields": [
        {
          "label": "Current asset",
          "value": "deduplication-rules_draft.sql"
        },
        {
    ...
    {
      "type": "action-rationale",
      "id": "deduplication-rules-scenario-3",
      "title": "Explain why the result is trustworthy",
      "context": "You are presenting deduplication rules in a team review. Choose the explanation that best demonstrates sound analytics judgment.",
      "displayFields": [
        {
          "label": "Audience",
  ...
  "hints": [
    "Start by naming the business grain that deduplication rules should produce.",
  ...
  "careerInsight": "Pipeline teams create trust at scale by spotting data quality risk before it turns into broken reporting or missed SLAs. Deduplication Rules is a common scenario in interviews, production reviews, and cross-functional planning.",
  "toolRelevance": [
    "dbt",
    "Airflow",
    "BigQuery"
