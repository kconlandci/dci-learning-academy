...
export const materializationVsComputeLab: LabManifest = LabManifestSchema.parse({
  "schemaVersion": "1.1",
  "id": "materialization-vs-compute",
  "version": 1,
  "title": "Materialization vs Compute",
  "tier": "beginner",
  ...
  "description": "Practice materialization vs compute inside a realistic analytics workflow, choose the safest decision, and explain why it is the most trustworthy SQL or data move.",
  "estimatedMinutes": 10,
  "learningObjectives": [
    "Clarify the stakeholder need behind materialization vs compute.",
  ...
  "scenarios": [
    {
      "type": "toggle-config",
      "id": "materialization-vs-compute-scenario-1",
      "title": "Configure the default quality controls",
      "description": "You are preparing a shared asset for materialization vs compute. Set the defaults so the platform SRE gets trustworthy results without relying on tribal knowledge.",
      "targetSystem": "warehouse.query_history.semantic_model",
      "items": [
        {
          "id": "grain-check",
      ...
      "feedback": {
        "perfect": "Great configuration. Materialization vs Compute becomes safer to reuse when quality checks, null policies, and reviews are explicit.",
    ...
    {
      "type": "toggle-config",
      "id": "materialization-vs-compute-scenario-2",
      "title": "Set access and change controls",
      "description": "A wider audience wants self-service access to materialization vs compute. Configure the model so speed does not outrun governance.",
      "targetSystem": "warehouse.query_history.reporting_view",
      "items": [
        {
          "id": "access-scope",
          "label": "Access Scope",
    ...
    {
      "type": "toggle-config",
      "id": "materialization-vs-compute-scenario-3",
      "title": "Tune the publish workflow",
      "description": "The team wants a faster release cycle for materialization vs compute. Configure the publish workflow so the team can move quickly without normalizing broken output.",
      "targetSystem": "warehouse.query_history.publish_job",
      "items": [
        {
          "id": "test-suite",
  ...
  "hints": [
    "Start by naming the business grain that materialization vs compute should produce.",
  ...
  "careerInsight": "Advanced analytics work often separates solid SQL users from the engineers who can keep an entire warehouse fast and stable. Materialization vs Compute is a common scenario in interviews, production reviews, and cross-functional planning.",
  "toolRelevance": [
    "EXPLAIN plans",
    "Snowflake",
    "PostgreSQL"
