...
export const incrementalLoadsLab: LabManifest = LabManifestSchema.parse({
  "schemaVersion": "1.1",
  "id": "incremental-loads",
  "version": 1,
  ...
  "description": "Practice incremental loads inside a realistic analytics workflow, choose the safest decision, and explain why it is the most trustworthy SQL or data move.",
  "estimatedMinutes": 14,
  "learningObjectives": [
    "Clarify the stakeholder need behind incremental loads.",
  ...
  "scenarios": [
    {
      "type": "toggle-config",
      "id": "incremental-loads-scenario-1",
      "title": "Configure the default quality controls",
      "description": "You are preparing a shared asset for incremental loads. Set the defaults so the data ops lead gets trustworthy results without relying on tribal knowledge.",
      "targetSystem": "orchestration.pipeline_runs.semantic_model",
      "items": [
        {
          "id": "grain-check",
      ...
      "feedback": {
        "perfect": "Great configuration. Incremental Loads becomes safer to reuse when quality checks, null policies, and reviews are explicit.",
    ...
    {
      "type": "toggle-config",
      "id": "incremental-loads-scenario-2",
      "title": "Set access and change controls",
      "description": "A wider audience wants self-service access to incremental loads. Configure the model so speed does not outrun governance.",
      "targetSystem": "orchestration.pipeline_runs.reporting_view",
      "items": [
        {
          "id": "access-scope",
          "label": "Access Scope",
    ...
    {
      "type": "toggle-config",
      "id": "incremental-loads-scenario-3",
      "title": "Tune the publish workflow",
      "description": "The team wants a faster release cycle for incremental loads. Configure the publish workflow so the team can move quickly without normalizing broken output.",
      "targetSystem": "orchestration.pipeline_runs.publish_job",
      "items": [
        {
          "id": "test-suite",
  ...
  "hints": [
    "Start by naming the business grain that incremental loads should produce.",
  ...
  "careerInsight": "Pipeline teams create trust at scale by spotting data quality risk before it turns into broken reporting or missed SLAs. Incremental Loads is a common scenario in interviews, production reviews, and cross-functional planning.",
  "toolRelevance": [
    "dbt",
    "Airflow",
    "BigQuery"
