...
export const reconciliationQueriesLab: LabManifest = LabManifestSchema.parse({
  "schemaVersion": "1.1",
  "id": "reconciliation-queries",
  "version": 1,
  ...
  "description": "Practice reconciliation queries inside a realistic analytics workflow, choose the safest decision, and explain why it is the most trustworthy SQL or data move.",
  "estimatedMinutes": 14,
  "learningObjectives": [
    "Clarify the stakeholder need behind reconciliation queries.",
  ...
  "scenarios": [
    {
      "type": "toggle-config",
      "id": "reconciliation-queries-scenario-1",
      "title": "Configure the default quality controls",
      "description": "You are preparing a shared asset for reconciliation queries. Set the defaults so the platform manager gets trustworthy results without relying on tribal knowledge.",
      "targetSystem": "staging.raw_orders.semantic_model",
      "items": [
        {
          "id": "grain-check",
      ...
      "feedback": {
        "perfect": "Great configuration. Reconciliation Queries becomes safer to reuse when quality checks, null policies, and reviews are explicit.",
    ...
    {
      "type": "toggle-config",
      "id": "reconciliation-queries-scenario-2",
      "title": "Set access and change controls",
      "description": "A wider audience wants self-service access to reconciliation queries. Configure the model so speed does not outrun governance.",
      "targetSystem": "staging.raw_orders.reporting_view",
      "items": [
        {
          "id": "access-scope",
          "label": "Access Scope",
    ...
    {
      "type": "toggle-config",
      "id": "reconciliation-queries-scenario-3",
      "title": "Tune the publish workflow",
      "description": "The team wants a faster release cycle for reconciliation queries. Configure the publish workflow so the team can move quickly without normalizing broken output.",
      "targetSystem": "staging.raw_orders.publish_job",
      "items": [
        {
          "id": "test-suite",
  ...
  "hints": [
    "Start by naming the business grain that reconciliation queries should produce.",
  ...
  "careerInsight": "Pipeline teams create trust at scale by spotting data quality risk before it turns into broken reporting or missed SLAs. Reconciliation Queries is a common scenario in interviews, production reviews, and cross-functional planning.",
  "toolRelevance": [
    "dbt",
    "Airflow",
    "BigQuery"
