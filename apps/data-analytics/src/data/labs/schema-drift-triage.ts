...
export const schemaDriftTriageLab: LabManifest = LabManifestSchema.parse({
  "schemaVersion": "1.1",
  "id": "schema-drift-triage",
  "version": 1,
  "title": "Schema Drift Triage",
  "tier": "beginner",
  ...
  "description": "Practice schema drift triage inside a realistic analytics workflow, choose the safest decision, and explain why it is the most trustworthy SQL or data move.",
  "estimatedMinutes": 10,
  "learningObjectives": [
    "Clarify the stakeholder need behind schema drift triage.",
  ...
  "scenarios": [
    {
      "type": "toggle-config",
      "id": "schema-drift-triage-scenario-1",
      "title": "Configure the default quality controls",
      "description": "You are preparing a shared asset for schema drift triage. Set the defaults so the analytics engineer gets trustworthy results without relying on tribal knowledge.",
      ...
      "feedback": {
        "perfect": "Great configuration. Schema Drift Triage becomes safer to reuse when quality checks, null policies, and reviews are explicit.",
    ...
    {
      "type": "toggle-config",
      "id": "schema-drift-triage-scenario-2",
      "title": "Set access and change controls",
      "description": "A wider audience wants self-service access to schema drift triage. Configure the model so speed does not outrun governance.",
    ...
    {
      "type": "toggle-config",
      "id": "schema-drift-triage-scenario-3",
      "title": "Tune the publish workflow",
      "description": "The team wants a faster release cycle for schema drift triage. Configure the publish workflow so the team can move quickly without normalizing broken output.",
  ...
  "hints": [
    "Start by naming the business grain that schema drift triage should produce.",
  ...
  "careerInsight": "Pipeline teams create trust at scale by spotting data quality risk before it turns into broken reporting or missed SLAs. Schema Drift Triage is a common scenario in interviews, production reviews, and cross-functional planning.",
  "toolRelevance": [
    "dbt",
    "Airflow",
    "BigQuery"
