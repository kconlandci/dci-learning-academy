...
`,
  ...
  "data-analytics": {
    sourceRepoName: "DataForge",
    destSlug: "data-analytics",
    textReplacements: [
      ...
      // Phase B: inject Firestore mirror after mirrorToPreferences
      {
        ...
        replacement:
          ...
          "$1    : null;\n" +
          "$1if (dciStudentId) {\n" +
          "$1  recordToFirestore(dciStudentId, \"data-analytics\", labId).catch(\n" +
          "$1    (err) => {\n" +
          "$1      console.warn(\n" +
          "$1        \"[DCI] Failed to record lab completion to Firestore:\",\n" +
          "$1        err,\n" +
...
`,
      ...
      "src/App.tsx": `// Stubbed by scripts/rebrand.ts — gutted version of the original DataForge App.tsx.
import { lazy, Suspense } from "react";
import { BrowserRouter, Routes, Route, useLocation } from "react-router-dom";
import HomeScreen from "./screens/HomeScreen";
import LabScreen from "./screens/LabScreen";
...
export default function App() {
  return (
    <ErrorBoundary>
      <AuthProvider>
        <BrowserRouter basename="/dci-learning-academy/data-analytics">
          <AppLayout />
        </BrowserRouter>
      </AuthProvider>
    </ErrorBoundary>
...
const PORTAL_ROOT =
  import.meta.env.BASE_URL.replace(/\\/data-analytics\\/?$/, "/") || "/";
...
export default defineConfig(({ command }) => ({
  plugins: [react(), tailwindcss()],
  base: command === "build" ? "/dci-learning-academy/data-analytics/" : "/",
  build: {
    outDir: "dist",
    sourcemap: true,
  },
...
`,
  ...
  "it-support": {
    ...
    textReplacements: [
      ...
      { pattern: /^[ \t]*\/\/ --- PREMIUM.*\r?\n/gm, replacement: "" },
      // Phase B: inject @dci/shared import into useProgress.ts.
      // TechForge (unlike the other Forge apps) defines getLocalDateString
      // inline in useProgress.ts rather than importing it from a utils
      // file, so the standard localDate-import anchor doesn't match. Anchor
      // instead on the Preferences import which is unique to useProgress.ts.
      {
        pattern: /import \{ Preferences \} from "\.\.\/capacitor-shim";/,
        replacement:
      ...
      // Phase B: inject Firestore mirror after mirrorToPreferences
      {
        ...
        replacement:
          ...
          "$1    : null;\n" +
          "$1if (dciStudentId) {\n" +
          "$1  recordToFirestore(dciStudentId, \"it-support\", labId).catch(\n" +
          "$1    (err: unknown) => {\n" +
          "$1      console.warn(\n" +
          "$1        \"[DCI] Failed to record lab completion to Firestore:\",\n" +
          "$1        err,\n" +
...
`,
      ...
      "src/App.tsx": `// Stubbed by scripts/rebrand.ts — gutted version of the original TechForge App.tsx.
import { lazy, Suspense } from "react";
import { BrowserRouter, Routes, Route, useLocation } from "react-router-dom";
import HomeScreen from "./screens/HomeScreen";
import LabScreen from "./screens/LabScreen";
...
export default function App() {
  return (
    <ErrorBoundary>
      <AuthProvider>
        <BrowserRouter basename="/dci-learning-academy/it-support">
          <AppLayout />
        </BrowserRouter>
      </AuthProvider>
    </ErrorBoundary>
...
const PORTAL_ROOT =
  import.meta.env.BASE_URL.replace(/\\/it-support\\/?$/, "/") || "/";
...
function required(key: keyof ImportMetaEnv): string {
  ...
  if (typeof value !== "string" || value.length === 0) {
    throw new Error(
      \`Missing required env var \${key}. Copy apps/it-support/.env.example to apps/it-support/.env and fill in the Firebase console values.\`,
    );
  }
  return value;
}
...
export default defineConfig(({ command }) => ({
  plugins: [react(), tailwindcss()],
  base: command === "build" ? "/dci-learning-academy/it-support/" : "/",
  build: {
    outDir: "dist",
    sourcemap: true,
  },
